...
export default function ManageApisTransferList({
	...
	useEffect(() => {
		if (testForEdit != null) {
			const projectListAPIs = testForEdit.listAPIs.map((api) => api.name);
	...
	const updateListAPIS = (id) => {
		...
		if (removedAPIS.length !== 0) {
			dispatch(removeApisFromTest(id, { apis: removedAPIS }));
		}
		if (addedAPIs.length !== 0) {
			dispatch(addApisToTest(id, { apis: addedAPIs }));
		}
		setLoading(false);
